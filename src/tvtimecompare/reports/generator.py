...
def generate_reports(result: ComparisonResult, output_dir: Path) -> ReportPaths:
    ...
    _write_html_report(result, paths.report_html)
    return paths


def _write_summary_csv(result: ComparisonResult, path: Path) -> None:
...
def _write_html_report(result: ComparisonResult, path: Path) -> None:
